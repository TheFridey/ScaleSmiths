...
try {
  ...
  await db.connect()
  try {
    const { rows: tasks } = await db.query("select id, project_id, status, result_quality, provider from forge_tasks where project_id=$1 order by id", [id])
    const fallback = tasks.filter((task) => task.result_quality === "fallback" || task.provider === "mock")
    ...
    for (const action of ["research_completed", "research_approved", "copy_rejected", "copy_approved", "design_system_completed", "design_system_approved", "workspace_created", "qa_completed", "proposal_generated"]) assert.ok(logs.some((log) => log.action === action), `missing activity log: ${action}`)
