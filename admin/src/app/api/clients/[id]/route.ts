...
const CLIENT_STATUSES = new Set(["active", "build", "review", "prospect"])
...
export async function PATCH(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (!status || !CLIENT_STATUSES.has(status)) return NextResponse.json({ error: "Select a valid client status." }, { status: 400 })
