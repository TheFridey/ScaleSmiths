...
export async function POST(_request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const outcome = await enqueueForgeJob({ projectId, kind: "consistency_review", actor })
    ...
    return NextResponse.json({ error: "Unable to evaluate cross-artifact consistency." }, { status: 500 })
  }
}
