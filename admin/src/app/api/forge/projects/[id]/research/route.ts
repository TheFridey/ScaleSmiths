...
export async function POST(_request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    const outcome = await enqueueForgeJob({ projectId, kind: "research", actor: sessionActor(session) })
    return NextResponse.json(forgeJobResponseBody(outcome))
  } catch (error) {
    if (error instanceof ForgeResearchAgentError) {
    ...
    return NextResponse.json({ error: "Unable to run Forge research agent." }, { status: 500 })
  }
}
