import { cancelForgeRun } from "@/lib/server/forge-run-orchestrator"
...
export async function POST(request: Request, { params }: { params: Promise<{ runId: string }> }) {
  try {
    ...
    return Response.json({ ok: true, run: await cancelForgeRun(parsePositiveId((await params).runId, "run id"), actor, String(body.reason ?? "")) })
