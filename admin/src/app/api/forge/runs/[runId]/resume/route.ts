import { resumeForgeRun } from "@/lib/server/forge-run-orchestrator"
...
export async function POST(request: Request, { params }: { params: Promise<{ runId: string }> }) {
  try {
    ...
    return Response.json({ ok: true, run: await resumeForgeRun(parsePositiveId((await params).runId, "run id"), actor, override) })
