...
import { revokePortalTokens, updatePortalUser } from "@/lib/server/portal-users"
...
export async function PATCH(request: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    if (input.revokeTokens === true) {
      const actor = await guardApiCapability("portal_users.credentials.reset")
      return NextResponse.json({ ok: true, user: await revokePortalTokens(id, actor) })
    }
    ...
    if (credentialChange) throw new PortalUserError("Use a secure activation or reset invitation; plaintext credential resets are disabled.")
    const actor = await guardApiCapability("portal_users.manage")
    return NextResponse.json({ ok: true, user: await updatePortalUser(id, input, actor) })
  } catch (error) {
