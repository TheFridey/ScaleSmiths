...
import { createDeliveryMilestone } from "@/lib/server/delivery-project-service"
...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    return NextResponse.json({ ok: true, milestone: await createDeliveryMilestone(projectId((await params).id), await projectPayload(request), actor) }, { status: 201 })
