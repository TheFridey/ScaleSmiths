...
export function AdminUsersManager({ initialUsers, actorRole, embedded = false }: { initialUsers: UserRow[]; actorRole: AdminRole; embedded?: boolean }) {
  ...
  return <section className={`${embedded ? "space-y-5 rounded-xl border border-b1 bg-s1 p-4 sm:p-5" : "mx-auto max-w-6xl space-y-6 p-3"}`}>
    ...
    <form onSubmit={createUser} className="grid gap-3 rounded-xl border border-b1 bg-s1 p-4 md:grid-cols-5">
      ...
      <select aria-label="Role" value={form.role} onChange={(e) => setForm({ ...form, role: e.target.value as AdminRole })}>{ADMIN_ROLES.filter((role) => canUseControl(actorRole, "admin_users.owner.assign") || (role !== "owner" && role !== "venture_controller")).map((role) => <option key={role}>{role}</option>)}</select>
      <button disabled={busy} className="rounded-lg bg-acc px-4 py-2 font-medium text-white disabled:opacity-50">Create user</button>
    </form>
    <div className="overflow-x-auto rounded-xl border border-b1 bg-s1"><table className="w-full text-left text-sm"><thead><tr className="border-b border-b1 text-t2"><th className="p-3">User</th><th className="p-3">Role</th><th className="p-3">Status</th><th className="p-3">Last login</th><th className="p-3">Actions</th></tr></thead><tbody>{users.map((user) => <tr key={user.id} className="border-b border-b1/60"><td className="p-3"><div className="font-medium">{user.displayName}</div><div className="text-xs text-t2">{user.email}</div></td><td className="p-3"><select value={user.role} disabled={busy || (!canUseControl(actorRole, "admin_users.owner.assign") && (user.role === "owner" || user.role === "venture_controller"))} onChange={(e) => updateUser(user.id, { role: e.target.value })}>{ADMIN_ROLES.filter((role) => canUseControl(actorRole, "admin_users.owner.assign") || (role !== "owner" && role !== "venture_controller")).map((role) => <option key={role}>{role}</option>)}</select></td><td className="p-3">{user.active ? "Active" : "Disabled"}{user.mfaEnabled ? " · MFA" : ""}</td><td className="p-3 text-t2">{user.lastLoginAt ? new Date(user.lastLoginAt).toLocaleString() : "Never"}</td><td className="p-3"><div className="flex flex-wrap gap-2"><button disabled={busy} onClick={() => updateUser(user.id, { active: !user.active })} className="rounded border border-b1 px-2 py-1">{user.active ? "Disable" : "Enable"}</button><button disabled={busy} onClick={() => updateUser(user.id, { revokeSessions: true })} className="rounded border border-b1 px-2 py-1">Revoke sessions</button>{canUseControl(actorRole, "admin_users.credentials.reset") && <button disabled={busy} onClick={() => setPasswordReset({ userId: user.id, password: "" })} className="rounded border border-b1 px-2 py-1">Reset password</button>}{user.mfaEnabled && canUseControl(actorRole, "admin_users.credentials.reset") && <button disabled={busy} onClick={() => setMfaReset({ userId: user.id, ownerPassword: "" })} className="rounded border border-red-500/40 px-2 py-1 text-red-300">Invalidate MFA</button>}</div></td></tr>)}</tbody></table></div>
  </section>
}
