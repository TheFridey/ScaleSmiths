...
export const AUTHORIZATION_POLICY = [
  ...
  rule({ id: "prospect-conversion", route: /^\/api\/prospects\/[^/]+\/conversion$/, methods: ["GET", "POST"], domain: "sales", capability: (method) => method === "GET" ? "leads.read" : "prospects.convert", scope: "global", scopeRule: "Prospect is locked and must be won; one durable conversion record owns all idempotent outputs.", authenticated: true, allow: "Preview requires leads.read; execution requires prospects.convert.", deny: "401/403; non-won, mismatched, duplicate or invalid linked records are rejected." }),
  ...
  rule({ id: "delivery-projects", route: /^\/api\/projects(?:\/[^/]+)?(?:\/(?:milestones|deliverables|onboarding-items|resources|documents|decisions|forge-integration)(?:\/[^/]+)?)?$/, methods: ["GET", "POST", "PATCH"], domain: "delivery", capability: readWrite("projects.read", "projects.write"), scope: "client", scopeRule: "Every nested record is resolved through its delivery project and owning client; Forge linkage is internal-only and client projection is sanitised.", authenticated: true, allow: "Required project capability and ownership checks succeed.", deny: "401/403; unknown or cross-project identifiers and invalid lifecycle transitions are rejected." }),
