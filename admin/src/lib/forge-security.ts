...
export function isForgeTaskEndpoint(pathname: string) {
  return /^\/api\/forge\/ai\/test$/.test(pathname) ||
    /^\/api\/forge\/projects\/[^/]+\/(?:command-chat|component-spec|copy|deploy|design|export|generate-site|preview|proposal|qa|research|seo|sitemap|visual-qa|workspace)(?:\/)?$/.test(pathname)
}
