...
export async function loadClientAnalyticsSummary(clientId: number): Promise<ClientAnalyticsSummary> {
  ...
  return withClientTenant(clientId, async (db) => {
...
export async function saveGeneratedOptimisationProposal(input: { clientId: number; key: string; actor: string }) {
  ...
  return withClientTenant(input.clientId, async (db) => {
...
export async function ingestClientAnalytics(clientId: number, configId: number, actor: string) {
  ...
  const config = await withClientTenant(clientId, async (db) => {
    const [row] = await db.select().from(clientAnalyticsConfigs).where(and(eq(clientAnalyticsConfigs.id, configId), eq(clientAnalyticsConfigs.clientId, clientId))).limit(1)
    return row
  })
  if (!config) throw new Error("Analytics configuration not found.")
  if (!config.enabled || !config.consentGranted) {
    await withClientTenant(clientId, async (db) => {
  ...
  await withClientTenant(clientId, async (db) => {
    for (const metric of minimised) {
      if (metric.clientId !== clientId) throw new Error("Analytics adapter returned metrics for a different client tenant.")
    ...
    await db.update(clientAnalyticsConfigs).set({ lastIngestedAt: new Date(), updatedBy: actor, updatedAt: new Date() }).where(and(eq(clientAnalyticsConfigs.id, config.id), eq(clientAnalyticsConfigs.clientId, clientId)))
