...
export function runWithForgeAttribution<T>(attribution: Partial<ForgeAttribution>, callback: () => T): T {
  ...
  return store.run(mergeAttribution(parent, attribution), callback)
}

/** Alias kept for call sites that prefer the shorter name. Same ALS scope. */
export const withForgeAttribution = runWithForgeAttribution
