...
export async function crawlForgeExistingSite(start: string, options: ForgeSiteCrawlerOptions = {}, dependencies: Partial<CrawlerDependencies> = {}): Promise<ForgeSiteInventory> {
  const deps: CrawlerDependencies = {
    fetch: dependencies.fetch ?? fetch,
    resolve: dependencies.resolve ?? (async (hostname) => (await lookup(hostname, { all: true, verbatim: true })).map((item) => item.address)),
  ...
  await assertSafePublicUrl(startUrl, allowedDomains, deps.resolve)
  ...
  if (config.robots === "respect") {
    try {
      const result = await fetchBounded(robotsUrl, allowedDomains, config, deps)
      robotsStatus = result.response.status
      if (result.response.ok) robots = result.body
    } catch (error) {
      failures.push(failure(robotsUrl.toString(), 0, "robots_fetch", error, deps.now()))
    }
  }
  ...
  while (queue.length && pages.length < config.maxPages) {
    ...
    if (config.robots === "respect" && isRobotsDisallowed(item.url, robots)) {
      failures.push({ url: item.url.toString(), depth: item.depth, category: "robots_disallowed", message: "Blocked by the configured robots policy.", occurredAt: deps.now().toISOString() })
      continue
    }
    try {
      const fetched = await fetchBounded(item.url, allowedDomains, config, deps)
      const contentType = fetched.response.headers.get("content-type") ?? ""
      if (!contentType.toLowerCase().includes("text/html")) throw new Error("Response is not HTML.")
      const page = extractPage(item.url, fetched.finalUrl, item.depth, fetched.response, fetched.body, fetched.redirects, deps.now())
      pages.push(page)
      ...
      failures.push(failure(item.url.toString(), item.depth, classifyFailure(error), error, deps.now()))
    }
  }
...
async function fetchBounded(url: URL, allowed: Set<string>, config: { timeoutMs: number; maxResponseBytes: number; maxRedirects: number }, deps: CrawlerDependencies) {
  let current = url
  const redirects: Array<{ from: string; to: string; status: number }> = []
  for (let hop = 0; ; hop += 1) {
    await assertSafePublicUrl(current, allowed, deps.resolve)
    const controller = new AbortController()
    const timeout = setTimeout(() => controller.abort(), config.timeoutMs)
    let response: Response
    try { response = await deps.fetch(current, { redirect: "manual", signal: controller.signal, headers: { Accept: "text/html,text/plain;q=0.8", "User-Agent": USER_AGENT } }) }
    catch (error) { clearTimeout(timeout); throw error }
    if (REDIRECT_CODES.has(response.status)) {
      clearTimeout(timeout)
      if (hop >= config.maxRedirects) throw new Error("Redirect limit exceeded.")
      const location = response.headers.get("location")
      if (!location) throw new Error("Redirect response omitted its location.")
      const next = new URL(location, current)
      await assertSafePublicUrl(next, allowed, deps.resolve)
      redirects.push({ from: current.toString(), to: next.toString(), status: response.status })
      current = next
      continue
    }
    const declaredSize = Number(response.headers.get("content-length") ?? 0)
    if (declaredSize > config.maxResponseBytes) { clearTimeout(timeout); throw new Error("Response exceeds the configured size limit.") }
    let buffer: Uint8Array
    try { buffer = new Uint8Array(await response.arrayBuffer()) }
    finally { clearTimeout(timeout) }
    if (buffer.byteLength > config.maxResponseBytes) throw new Error("Response exceeds the configured size limit.")
    return { response, body: new TextDecoder().decode(buffer), finalUrl: current, redirects }
  }
}

export async function assertSafePublicUrl(url: URL, allowedDomains: Set<string>, resolve: (hostname: string) => Promise<string[]>) {
  if (!new Set(["http:", "https:"]).has(url.protocol) || url.username || url.password) throw new Error("Only credential-free HTTP and HTTPS URLs may be crawled.")
  ...
  if (!addresses.length || addresses.some(isBlockedAddress)) throw new Error("URL resolves to a blocked or unavailable network address.")
}
...
export function isBlockedAddress(address: string) {
  ...
  if (version === 6) return normalized === "::" || normalized === "::1" || normalized.startsWith("fc") || normalized.startsWith("fd") || normalized.startsWith("fe8") || normalized.startsWith("fe9") || normalized.startsWith("fea") || normalized.startsWith("feb") || normalized.startsWith("2001:db8:")
  return true
}

function extractPage(requestedUrl: URL, finalUrl: URL, depth: number, response: Response, html: string, redirects: ForgeSiteInventoryPage["redirects"], now: Date): ForgeSiteInventoryPage {
  ...
  return {
    requestedUrl: requestedUrl.toString(), finalUrl: finalUrl.toString(), depth, status: response.status, redirects,
    ...
    structuredData, contentType: response.headers.get("content-type") ?? "", contentBytes: new TextEncoder().encode(html).byteLength, fetchedAt: now.toISOString(),
  }
}
...
function failure(url: string, depth: number, category: string, error: unknown, now: Date) { return { url, depth, category, message: error instanceof Error ? error.message : "Crawl failed.", occurredAt: now.toISOString() } }
function classifyFailure(error: unknown) { const message = error instanceof Error ? error.message.toLowerCase() : ""; if (message.includes("blocked") || message.includes("approved domain")) return "security_block"; if (message.includes("size limit")) return "size_limit"; if (message.includes("redirect")) return "redirect"; if (message.includes("abort") || message.includes("timeout")) return "timeout"; return "fetch_failure" }
