...
describe("Forge invalidation modes (real PostgreSQL)", () => {
  it("standard mode: changing copy invalidates downstream design_direction, not design_system", async () => {
    ...
    await createStep(run, project, "design_direction", 4)
    await createStep(run, project, "design_system", 5)
    ...
    expect(steps.rows.find((step) => step.stage === "design_direction")?.status).toBe("pending")
    expect(steps.rows.find((step) => step.stage === "design_system")?.status).toBe("completed")
  })
  ...
  it("redesign mode invalidation", async () => {
    ...
    const { invalidateDownstreamForChangedInput } = await import("../../src/lib/server/forge-runs/invalidation")
    await invalidateDownstreamForChangedInput(run, project, "copy", "test")
    const design = await pool.query("SELECT status FROM forge_run_steps WHERE run_id=$1 AND stage='design_direction'", [run])
  ...
  it("refresh mode invalidation", async () => {
    ...
    const { invalidateDownstreamForChangedInput } = await import("../../src/lib/server/forge-runs/invalidation")
    await invalidateDownstreamForChangedInput(run, project, "copy", "test")
    expect((await pool.query("SELECT status FROM forge_run_steps WHERE run_id=$1 AND stage='design_direction'", [run])).rows[0].status).toBe("pending")
  })
  ...
  it("policy-disabled stages remain skipped", async () => {
    const project = await createProject()
    const run = await createRun(project, "standard", { skipStages: { design_direction: "Existing approved design direction is retained for this refresh." } })
    await insertArtifact(project, "handover_doc", "brief-v1")
    await insertArtifact(project, "copy_doc", "copy-v1")
    await createStep(run, project, "brief", 1)
    await createStep(run, project, "copy", 3)
    await createStep(run, project, "design_direction", 4, "skipped")
    await pool.query("UPDATE forge_run_steps SET output_artifact_ids='[99]' WHERE stage='copy' AND run_id=$1", [run])
    await insertArtifact(project, "copy_doc", "copy-v2")
    const { invalidateDownstreamForChangedInput } = await import("../../src/lib/server/forge-runs/invalidation")
    await invalidateDownstreamForChangedInput(run, project, "copy", "test")
  ...
  it("changing design does not invalidate upstream research/copy", async () => {
    ...
    await insertArtifact(project, "handover_doc", "brief-v1")
    await insertArtifact(project, "copy_doc", "copy-v1")
    await insertArtifact(project, "design_direction", "design-v1")
    await createStep(run, project, "brief", 1)
    await createStep(run, project, "research", 2)
    await createStep(run, project, "copy", 3)
    await createStep(run, project, "design_direction", 4)
    await pool.query("UPDATE forge_run_steps SET output_artifact_ids='[99]' WHERE stage='design_direction' AND run_id=$1", [run])
    await insertArtifact(project, "design_direction", "design-v2")
    const { invalidateDownstreamForChangedInput } = await import("../../src/lib/server/forge-runs/invalidation")
    await invalidateDownstreamForChangedInput(run, project, "design_direction", "test")
