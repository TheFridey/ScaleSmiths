...
describe("migration installation paths", () => {
  it("applies every migration from zero in production order with separate journals", async () => {
    ...
    expect(await migrationCount("__drizzle_migrations")).toBe(50)
    expect(await columnExists("forge_artifacts", "content_bytes")).toBe(true)
    expect(await columnExists("forge_ai_usage", "run_id")).toBe(true)
    expect(await columnExists("forge_ai_usage", "run_step_id")).toBe(true)
    expect(await columnExists("forge_ai_usage", "job_id")).toBe(true)
