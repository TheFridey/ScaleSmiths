...
test("mobile navigation sheet traps focus and restores the page", async ({ page }) => {
  ...
  await expect.poll(() => page.evaluate(() => ({ overflow: document.body.style.overflow, inert: [...document.body.children].some((element) => (element as HTMLElement).inert) }))).toEqual({ overflow: "", inert: false })
})
