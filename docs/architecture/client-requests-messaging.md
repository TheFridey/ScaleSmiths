...
Client-initiated conversation with ScaleSmiths — formal work requests and general portal messages alike — lives in `client_requests` / `client_request_messages` / `client_timeline_events`. A general portal message is a `client_requests` row with `category = "general_support"` and title `Portal messages`: one continuous, non-terminal thread per client, resolved or created on first send (`resolveGeneralMessageThreadId` in `web/src/lib/portal-client-requests.ts`).

The portal Messages tab is the client-visible inbox for those threads. It lists authorised request-thread history (`listPortalMessageThreads`) and opens a selected thread through the same `getPortalRequestThread` projection used by `/portal/[clientId]/requests/[requestId]`. Formal request intake remains on the Requests tab.

## Flow

1. Client sends a message from Messages (`web/src/components/portal/PortalMessagesPanel.tsx`) → `POST /portal/api/requests/[id]` for an existing thread, or `POST /portal/api/messages` to create/continue the general support thread.
...
6. The client sees the reply in Messages and in the request thread on next load. Internal (`visibility: "internal"`) notes are excluded from every portal query.

## Read state

`client_requests.client_last_read_at` is stamped when the portal opens a thread (Messages tab selection, the request thread page, and `GET /portal/api/requests/[id]`). `client_requests.admin_last_read_at` is stamped when admin selects a request in the queue (`PATCH /api/client-requests/[id]` with `action: "markRead"`). "Unread" is derived by comparing the latest client-visible staff message against `client_last_read_at` — no read-receipts table.

## Inbox MVP

Messages lists the 20 most recently active threads that have at least one client-visible message. Opening a thread loads its full client-visible history. Older conversations remain reachable from Requests. This is an explicit scoped MVP: there is no infinite-scroll inbox or per-message read cursor yet.
...
- A Messages `thread` query parameter for another client's request is ignored; the page still renders only the caller's inbox.
...
A `mailto:hello@scalesmiths.co.uk` link is shown in the portal composer only after a send attempt fails — never as the default action.
