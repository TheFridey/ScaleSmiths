...
- DNS is resolved before each request. Any result containing private, loopback, link-local, unique-local, carrier-grade NAT, benchmark, multicast, or otherwise non-public addresses is rejected, including cloud-metadata forms (`169.254.169.254`, `fd00:ec2::254`, and IPv4-mapped / NAT64 equivalents). Documentation ranges used as public stand-ins in tests remain allowed. The HTTP connection is bound to one validated address: TCP connects to that IP, TLS still verifies the original hostname, every redirect hop is independently revalidated, and the connected socket is rejected if its address does not match the pin or is forbidden. Production egress filtering remains defence in depth; see [Forge outbound egress policy](../operations/forge-egress-policy.md).
