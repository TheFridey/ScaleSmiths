...
The authoritative classifications and evidence are maintained in the [current residual-risk register](residual-risk-register.md). In particular, host-Nginx request behaviour, durable rate limiting, leased preview/container reconciliation, migration immutability, dependency admission, Forge run recovery and DNS-pinned Forge egress are now implemented. Production grant verification, tenant-wide RLS, monitoring activation, external smoke checks, complete authorization E2E coverage, and production-derived restore evidence remain partial or open as recorded there.
