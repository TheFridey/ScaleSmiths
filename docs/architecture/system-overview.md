...
| Shared database | PostgreSQL | Separate least-privilege production runtime roles; one migration owner applies both Drizzle histories through the shared dependency planner |
...
- `/about`: founder-led trust route. All founder copy is centralised in `web/src/lib/founders.ts`, where every published statement carries a repository evidence path and unevidenced biography detail is rendered as an explicit "awaiting founder confirmation" note rather than a claim. Optional founder contact links come from `NEXT_PUBLIC_FOUNDER_*` configuration and publish nothing when unset or non-`https:`/`mailto:`.
