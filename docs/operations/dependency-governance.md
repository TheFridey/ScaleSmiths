...
Admin remains on the reviewed `next-auth` `5.0.0-beta.32`. The 2026-09-17 issue #59 review confirmed that npm `latest` is still the older v4 line (`4.24.15`) and that no stable `5.0.0` exists, so changing tags is not a safe stable migration. The lockfile was inspected and left unchanged: it already resolves this exact pin and transitive `@auth/core@0.41.3` with registry-matching integrity. The machine-readable pre-release exception in `scripts/dependency-governance-policy.json` must match the exact manifest pin, stay unexpired (`reviewBy` is CI-enforced), and points to the full API inventory, rationale, advisory evidence, test coverage, checklist and exit criterion in `docs/security/authjs-beta-risk-acceptance.md`. The current acceptance is owned by the repository owner and is next due on 2027-01-30. Any Auth.js update must be deliberate and prove credentials login, MFA success/failure and recovery code use, middleware protection, secure cookie settings, role/session-version revocation, disabled-user rejection, logout and rate limiting.
