...
| Admin runtime (`ADMIN_DATABASE_URL`) | `CONNECT`; `USAGE` on `public`; no `CREATE`, `TEMPORARY`, or `drizzle` access | `SELECT`, `INSERT`, `UPDATE` on public application relations; `DELETE` only on the lifecycle tables declared in `postgres-privilege-policy.mjs`; no `TRUNCATE`, `REFERENCES`, or `TRIGGER` | `USAGE`/`SELECT` on public sequences; execute only the reviewed `digest` and `gen_random_uuid` functions when present | Ownership, DDL, role management, journal access, undeclared row deletion |
