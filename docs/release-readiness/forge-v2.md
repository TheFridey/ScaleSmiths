...
- Branch: `master`
- Final master SHA: `6fe922968bff3fa864b33ea998644dbaaccf3985`
...
| --- | --- | --- | --- |
| CI | `30644768940` | final master SHA | **Passed**; 9 executed jobs passed, PR Metadata skipped on push |
| Security | `30644768930` | final master SHA | **Passed**; 8 executed jobs passed, Dependency Review skipped on push |
| CodeQL | `30644768934` | final master SHA | **Passed**; JavaScript/TypeScript analysis |
| Dependency Review | Security `30639546348`, job `91185672417` | PR #38 head `d955e2c30ea2e7456403b80c3859ac53659a6787` into preceding master | **Passed** |

Dependency Review is a pull-request-only control. Its successful PR #38 result covers the final master's last dependency delta; the master push skip is expected and is not reported as a pass.
...
| Admin unit | 78 files, 546 tests passed |
...
The final master logs still contain:

- nine admin unused-code lint warnings;
- Node `DEP0040` (`punycode`) and `DEP0169` (`url.parse`) dependency warnings;
- Recharts 2.15.4 deprecation output;
- Sentry source-map upload warnings when no Sentry auth token is configured in CI.

These warnings are not silently reclassified as passes. None changed the recorded job result, and no remediation from an unmerged branch is credited here.

## Backup and recovery evidence

CI run `30644768940`, Backup and Restore job `91203312527`, passed against a synthetic disposable fixture. Artifact `backup-framework-test-log` ID `8798952137` records that framework test. It proves script safeguards, not recovery of production data.
