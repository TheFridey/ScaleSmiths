...
- Two npm contexts: run app commands from `admin/`. Typecheck is `npm exec tsc -- --noEmit` (no `npm run typecheck`). **This repo has PRE-EXISTING `tsc --noEmit` errors** (mostly `*.test.ts` / `test/` / `e2e/` files — `next build` does not typecheck those). The recorded baseline (refreshed after Task 1) is `.superpowers/sdd/2026-08-30-prospect-conversion-workflow/tsc-baseline.txt` — **31 errors** as of commit `3791388` (30 original + 1 verbatim-harness `NODE_ENV` line in Task 1's integration test). A task's tsc gate passes when it introduces **no new errors beyond that baseline** and the files it created/modified compile clean — not when the count is zero. Note tsc output text for a few pre-existing errors varies run-to-run (union-member ordering) without the error count changing. The authoritative typecheck gate is `npm run build` (Task 13). Per-task correctness comes from focused `npm test` / `npm run test:integration`.
- Migrations: **admin owns** `prospects`, `clients`, `prospect_conversions`, `client_service_assignments`. `portal_client_accounts` is **web-owned** — never generate an admin migration touching it; only admin *runtime* writes it (precedent: `provisionPortalAccount`).
