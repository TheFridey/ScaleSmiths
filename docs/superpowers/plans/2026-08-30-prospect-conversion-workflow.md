...
- Two npm contexts: run app commands from `admin/`. Typecheck is `npm exec tsc -- --noEmit` (no `npm run typecheck`). **This repo has ~30 PRE-EXISTING `tsc --noEmit` errors** (mostly `*.test.ts` / `test/` / `e2e/` files — `next build` does not typecheck those). The recorded baseline is `.superpowers/sdd/2026-08-29-prospect-conversion-workflow/tsc-baseline.txt`. A task's tsc gate passes when it introduces **no new errors beyond that baseline** and the files it created/modified compile clean — not when the count is zero. The authoritative typecheck gate is `npm run build` (Task 13). Per-task correctness comes from focused `npm test` / `npm run test:integration`.
