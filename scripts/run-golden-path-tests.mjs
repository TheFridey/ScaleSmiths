...
const compose = ["compose", "-p", "scalesmiths-golden-path", "-f", "docker-compose.integration-test.yml"]
...
try {
  if (!process.env.TEST_DATABASE_URL) await command("docker", [...compose, "up", "-d", "--wait"], root)
  const npmExecutable = process.platform === "win32" ? (process.env.ComSpec ?? "cmd.exe") : "npm"
  const npmArgs = process.platform === "win32" ? ["/d", "/s", "/c", "npm run test:golden-path"] : ["run", "test:golden-path"]
  await command(npmExecutable, npmArgs, path.join(root, "admin"), { ...process.env, TEST_DATABASE_URL: databaseUrl })
} finally {
  if (!process.env.TEST_DATABASE_URL) await command("docker", [...compose, "down", "--volumes"], root).catch((error) => console.error(`Golden Path database cleanup failed: ${error.message}`))
}
