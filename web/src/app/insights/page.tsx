...
export default function InsightsPage() {
  ...
  return <>
    ...
    <main>
      ...
      <section aria-labelledby="featured-insight" className="surface-paper border-y border-paper-border px-6 py-16 md:px-12"><div className="mx-auto grid max-w-[1240px] gap-8 lg:grid-cols-[.72fr_1.28fr] lg:items-center"><div><p className="text-xs font-semibold uppercase tracking-[.14em] text-paper-acc">Featured insight</p><h2 id="featured-insight" className="mt-3 font-syne text-[clamp(30px,4.5vw,52px)] font-extrabold tracking-[-.03em] text-paper-ink">{featured.title}</h2><p className="mt-5 max-w-[680px] text-base leading-relaxed text-paper-text">{featured.description}</p><Link href={`/insights/${featured.slug}`} prefetch={false} className="btn-primary mt-7">Read the guide <ArrowRight size={16} aria-hidden="true" /></Link></div><div className="rounded-3xl border border-paper-border bg-paper-raised p-8 md:p-12"><p className="font-syne text-2xl font-bold text-paper-ink">A practical starting point</p><p className="mt-4 text-sm leading-[1.8] text-paper-text">Start with scope, ownership and the problem the website or system must solve. The related guides below help you compare platforms, timelines and ongoing responsibilities without pretending one answer fits every business.</p><div className="mt-7 flex flex-wrap gap-3"><Link href="/services" className="btn-ghost">Explore services</Link><Link href="/work" className="btn-ghost">See delivered work</Link></div></div></div></section>
