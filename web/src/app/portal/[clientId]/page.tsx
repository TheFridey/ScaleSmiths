...
async function ProgressTab({ clientId }: { clientId: string }) {
  const [projects, invoices] = await Promise.all([listPortalProjectProgress(clientId), listPortalInvoices(clientId)])
  ...
  return (
    <div className="space-y-5">{projects.map((project) => <section key={project.id} className="rounded-2xl border border-b1 bg-s1 p-6">
      <div className="flex flex-wrap items-start justify-between gap-4"><div><div className="font-dm text-xs font-semibold uppercase tracking-[0.12em] text-acc">{project.currentPhase.replaceAll("_", " ")} phase</div><h2 className="mt-2 font-syne text-2xl font-bold">{project.name}</h2><p className="mt-2 max-w-3xl font-dm text-sm leading-relaxed text-t2">{project.summary || "Delivery milestones and progress for this project."}</p></div><span className="rounded border border-b2 bg-s2 px-2 py-1 font-dm text-xs text-t2">{project.clientStatus.replaceAll("_", " ")}</span></div>
      {project.portalWelcomeTitle || project.portalWelcomeContent ? <div className="mt-5 rounded-xl border border-acc/25 bg-acc/10 p-4"><h3 className="font-syne text-lg font-bold">{project.portalWelcomeTitle || "Welcome to your project"}</h3>{project.portalWelcomeContent ? <p className="mt-1 font-dm text-sm leading-relaxed text-t2">{project.portalWelcomeContent}</p> : null}</div> : null}
      <div className="mt-5 grid gap-3 sm:grid-cols-2"><div className="rounded-xl border border-b1 bg-s2 p-4"><div className="font-dm text-xs uppercase tracking-[0.1em] text-t3">Status</div><div className="mt-1 font-syne text-lg font-bold">{project.clientStatus.replaceAll("_", " ")}</div></div><div className="rounded-xl border border-b1 bg-s2 p-4"><div className="font-dm text-xs uppercase tracking-[0.1em] text-t3">Next step</div><div className="mt-1 font-dm text-sm font-semibold">{project.clientNextStep || "ScaleSmiths will share the next update here."}</div></div></div>
      {project.stagingUrl ? <a href={project.stagingUrl} target="_blank" rel="noreferrer" className="mt-4 inline-flex items-center gap-2 rounded-lg border border-acc/40 bg-acc/10 px-4 py-2 font-dm text-sm font-semibold text-acc">View staging website <ExternalLink size={14} /></a> : null}
      ...
      {project.onboardingItems.length ? <div className="mt-6"><h3 className="font-syne text-lg font-bold">What we need and what comes next</h3><div className="mt-3 grid gap-3 sm:grid-cols-2">{project.onboardingItems.filter((item) => item.status !== "not_required").map((item) => <article key={item.id} className="rounded-xl border border-b1 bg-s2 p-4"><div className="flex items-start justify-between gap-3"><div><div className="font-dm text-[11px] uppercase tracking-[0.08em] text-t3">{item.kind.replaceAll("_", " ")}</div><h4 className="mt-1 font-dm text-sm font-semibold">{item.title}</h4></div><span className="rounded border border-b2 px-2 py-0.5 font-dm text-[11px] text-t2">{item.status.replaceAll("_", " ")}</span></div>{item.description ? <p className="mt-2 font-dm text-xs text-t2">{item.description}</p> : null}{item.status === "blocked" && item.blocker ? <p className="mt-2 font-dm text-xs text-red-300">Blocked: {item.blocker}</p> : null}{item.nextAction ? <p className="mt-2 font-dm text-xs text-t2"><strong>Next:</strong> {item.nextAction}</p> : null}</article>)}</div></div> : null}
      {invoices.some((invoice) => invoice.projectId === project.id) ? <div className="mt-6"><h3 className="font-syne text-lg font-bold">Project invoices</h3><div className="mt-3 space-y-2">{invoices.filter((invoice) => invoice.projectId === project.id).map((invoice) => <Link key={invoice.invoiceNumber} href={`/portal/${clientId}/invoices/${encodeURIComponent(invoice.invoiceNumber)}`} className="flex items-center justify-between rounded-xl border border-b1 bg-s2 p-3 font-dm text-sm"><span>{invoice.invoiceNumber}</span><span>{INVOICE_STATUS_LABELS[invoice.status]} · {gbp(invoice.total)}</span></Link>)}</div></div> : null}
...
async function DocumentsTab({ clientId }: { clientId: string }) {
  ...
  return (
    <section className="rounded-2xl border border-b1 bg-s1 p-6"><FileText size={18} className="mb-4 text-acc" aria-hidden="true" /><h2 className="font-syne text-xl font-bold">Documents and assets</h2><p className="mt-1 font-dm text-sm text-t2">Files, previews, checklists and handoff resources published by ScaleSmiths.</p>{resources.length ? <div className="mt-6 grid gap-3 lg:grid-cols-2">{resources.map((resource) => <a key={resource.id} href={`/portal/api/documents/${resource.id}`} target="_blank" rel="noreferrer" className="flex items-center justify-between gap-3 rounded-xl border border-b1 bg-s2 p-4 hover:border-acc/60"><div><div className="font-dm text-xs text-t3">{resource.projectName} · {resource.documentType.replaceAll("_", " ")}</div><div className="mt-1 font-dm text-sm font-semibold text-t1">{resource.title}</div>{resource.description ? <p className="mt-1 font-dm text-xs text-t2">{resource.description}</p> : null}<div className="mt-2 font-dm text-[11px] text-t3">Version {resource.version}{resource.sizeBytes ? ` · ${(resource.sizeBytes / 1024).toFixed(1)} KB` : " · External link"}</div></div><ExternalLink size={15} className="text-acc" /></a>)}</div> : <div className="mt-6 rounded-xl border border-dashed border-b2 bg-s2 p-5"><div className="font-dm text-sm font-semibold text-t1">No documents published yet</div><p className="mt-1 font-dm text-sm text-t2">Briefs, proposals, brand assets, staging links, launch checklists and handoff notes will appear here when they are ready for you.</p></div>}</section>
  )
}
