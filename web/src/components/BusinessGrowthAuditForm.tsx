...
export function BusinessGrowthAuditForm() {
  ...
  return <form onSubmit={submit} onFocusCapture={markStarted} noValidate className="space-y-10">
    ...
    <FormSection number="04" title="Goals"><fieldset><legend className="mb-3 text-sm text-t2">Primary goal over the next 6–12 months <span className="text-acc">*</span></legend><div className="grid gap-2 sm:grid-cols-2">{goals.map((goal) => <label key={goal} className={`min-h-11 cursor-pointer rounded-xl border p-4 text-sm ${form.goal === goal ? "border-control-focus bg-acc/10 text-t1" : "border-control bg-s2 text-t2 hover:border-control-hover"}`}><input type="radio" name="goal" value={goal} checked={form.goal === goal} onChange={() => update("goal", goal)} className="mr-2 accent-[var(--acc)]" />{goal}</label>)}</div></fieldset><Area label="Anything else we should understand?" value={form.notes} update={(v) => update("notes", v)} optional /></FormSection>
...
const control = "field-control mt-2"
