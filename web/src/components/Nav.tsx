  ...
  return (
    <header
      className={cn(
        ...
        scrolled
          ? "border-b1/80 bg-bg/94 shadow-[0_12px_40px_rgba(0,0,0,.18)] backdrop-blur-md"
          : "border-transparent bg-bg/35",
      )}
    >
      <nav
      ...
      >
        ...
        <div className="ml-auto hidden items-center gap-7 md:flex">
          ...
          <Link
            href="/portal/login"
            prefetch={false}
            className="group inline-flex items-center gap-2 border-l border-b1 pl-5 font-dm text-[13px] font-medium text-t2 transition-colors hover:text-t1"
          >
        ...
        <button
          ref={toggleRef}
          type="button"
          className="ml-auto inline-flex min-h-11 min-w-11 items-center justify-center rounded-lg border border-b1 bg-s1/70 text-t1 md:hidden"
      ...
      <AnimatePresence initial={false}>
        {open && (
          <m.div
            className={cn("fixed inset-x-0 bottom-0 z-40 bg-black/55 md:hidden", scrolled ? "top-[62px]" : "top-[78px]")}
              ...
              <m.div variants={staggerContainer} initial={reducedMotion ? false : "hidden"} animate="visible" className="flex flex-col">
                {links.map((link) => (
                  <m.div key={link.href} variants={staggerItem}>
                    <Link href={link.href} prefetch={false} onClick={closeMenu} className="flex min-h-14 items-center border-b border-b1 font-syne text-xl font-bold text-t1">
                      {link.label}
                    </Link>
                  </m.div>
                ))}
