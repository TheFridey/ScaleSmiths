...
export const landingPages: Record<string, LandingPage> = {
  ...
  "custom-web-app-development-uk": {
    ...
    description: "Client portals, dashboards and SaaS platforms for UK businesses: secure logins, role-based access, billing and the infrastructure to run them properly.",
    eyebrow: "Custom web apps",
    h1: "Applications your customers, members or staff actually log into.",
    intro: "A web application is a product, not a bigger website. Accounts, permissions, billing, data and deployment all have to hold up on the day a real user signs in — so that is where the work goes.",
    searchIntent: "For UK businesses and founders who need a browser-based product with accounts and permissions — a client portal, a members area, a dashboard or a SaaS platform. If the problem is an internal process rather than a product, the custom software page is the better starting point.",
    location: "United Kingdom",
    serviceType: "Custom web app development",
    outcomes: ["Accounts and role-based access", "Billing that holds up in production", "A scoped first release", "Deployment somebody owns"],
    problems: ["Customers, clients or members need a secure place to sign in and do something repeatedly.", "A product idea needs a first release small enough to test but real enough to charge for.", "Different people need to see different things, and that cannot be handled by hiding buttons.", "Subscriptions, seats or usage need billing that reconciles correctly without manual work."],
    ...
    faqLibrary: ["customer-portals", "stripe", "internal-admin-systems", "software-scaling", "self-editing", "hosting-provided", "timeline", "support-included"],
  },
  ...
  "custom-software-development-uk": {
    ...
    description: "Bespoke software for UK businesses tied up in spreadsheets, manual handling and disconnected tools — including an honest answer on whether to build at all.",
    eyebrow: "Custom software · UK", h1: "Software for the operations behind the business, not the shop window.",
    intro: "Most requests for custom software start with a process that has outgrown its spreadsheet. We map what the work actually involves, say plainly whether an existing product would do the job, and build only the part that genuinely needs building.",
    searchIntent: "For UK organisations weighing bespoke software against an off-the-shelf product, usually because manual handling, duplicate data entry or a tool that does not fit is costing real time. If what you need is an application customers or staff log into, the custom web app page covers that instead.",
    location: "United Kingdom", serviceType: "Custom software development",
    outcomes: ["An honest build-or-buy answer", "Less duplicate data entry", "Systems that talk to each other", "A documented operational handover"],
    problems: ["The same information is typed into three places, and one of them is a spreadsheet nobody trusts.", "An off-the-shelf product forces the team into a workflow that does not match how the job is done.", "Two systems that should share data do not, so somebody reconciles them by hand every week.", "Nobody can say what a job cost, because the record of it is spread across an inbox, a calendar and a notebook."],
    examples: ["A pest-control business running customers, properties, quotations, appointments, jobs, visits and commercial service records in one private system — the Confirm-A-Kill CRM.", "A repair firm whose enquiry form feeds a quote management panel, so every incoming job has one place to live — the CSDS build.", "A membership platform where billing, access tiers and member administration are one system rather than three subscriptions — The Business Circle."],
    ...
    proofLinks: ["confirm-a-kill", "csds", "the-business-circle"], buildLogLinks: ["admin-dashboard-foundation", "security-hardening-pass", "scalesmiths-platform-build"], relatedPages: ["business-automation-nottingham", "custom-web-app-development-uk", "web-development-nottingham"],
    faqs: [{ q: "How is custom software priced?", a: "After discovery, around the workflows, integrations, risk and release boundary. We provide a written proposal rather than estimating from screen count alone." }, { q: "Who owns the software?", a: "Ownership, licences, third-party services and access are stated in the proposal and contract. We do not leave this to assumption." }, { q: "Can you replace spreadsheets gradually?", a: "Yes. A phased system can begin with the highest-friction workflow and leave stable existing processes in place until replacement is justified." }], faqLibrary: ["custom-software", "replace-spreadsheets", "crm", "process-automation", "unfinished-project", "code-ownership", "phases"],
  },
