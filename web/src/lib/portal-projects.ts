...
export async function listPortalProjectProgress(portalClientId: string) {
  const projects = await db.select({
    ...
    portalWelcomeTitle: portalDeliveryProjects.portalWelcomeTitle,
    portalWelcomeContent: portalDeliveryProjects.portalWelcomeContent,
  ...
  const [milestones, deliverables, onboardingItems, resources, decisions] = await Promise.all([
    ...
    db.select({ id: portalDeliveryOnboardingItems.id, projectId: portalDeliveryOnboardingItems.projectId, milestoneId: portalDeliveryOnboardingItems.milestoneId, kind: portalDeliveryOnboardingItems.kind, title: portalDeliveryOnboardingItems.title, description: portalDeliveryOnboardingItems.description, status: portalDeliveryOnboardingItems.status, blocker: portalDeliveryOnboardingItems.blocker, nextAction: portalDeliveryOnboardingItems.nextAction, targetDate: portalDeliveryOnboardingItems.targetDate, completedAt: portalDeliveryOnboardingItems.completedAt, position: portalDeliveryOnboardingItems.position })
      .from(portalDeliveryOnboardingItems).where(and(inArray(portalDeliveryOnboardingItems.projectId, projectIds), eq(portalDeliveryOnboardingItems.clientVisible, true))).orderBy(asc(portalDeliveryOnboardingItems.position)),
  ...
  return projects.map((project) => {
    ...
    return {
      ...
      onboardingItems: onboardingItems.filter((item) => item.projectId === project.id),
